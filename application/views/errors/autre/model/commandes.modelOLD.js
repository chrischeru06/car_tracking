...
const getAll = async (clientId, q, offset = 0, limit = 10) => {
          try {
                    var binds = [clientId]
                    var sqlQuery = "SELECT co.ID_COMMANDE, co.DATE_COMMANDE, co.ID_STATUS_COMMANDE, co.DATE_COMMANDE, co.NOM_LIVRAISON, co.PRENOM_LIVRAISON, co.NUMERO_LIVRAISON, co.POSITION_STATUS, co.ADRESSE_LIVRAISON, co.MONTANT_TOTAL, COUNT(cd.ID_COMMANDE) COMMANDES "
                    sqlQuery += "FROM commandes co "
                    sqlQuery += "LEFT JOIN commande_details cd ON cd.ID_COMMANDE = co.ID_COMMANDE "
                    sqlQuery += "WHERE 1 AND co.ID_CLIENT = ? GROUP BY cd.ID_COMMANDE "
