...
const getPromotions = async (req, res) => {
          try {
                    const allPromotions = await favoritesModel.getPromotions()
                    var promotions = []
                    allPromotions.forEach(async promotion => {
                              if(promotion.ID_RESTO) {
                                        const resto = await restoModel.getResto(promotion.ID_RESTO)
                              ...
                              if(promotion.ID_MENU_RESTO) {
                                        const menu = await menuModel.getMenu(promotion.ID_MENU_RESTO)
                                        const { IMAGE_1, IMAGE_2, IMAGE_3, ...other } = menu[0]
                                        var IMAGES = []
                                        if(IMAGE_1) IMAGES.push(IMAGE_1)
                                        if(IMAGE_2) IMAGES.push(IMAGE_2)
                                        if(IMAGE_3) IMAGES.push(IMAGE_3)
                              ...
                              // promotions = [...promotions, {...promotion, type: 'url'}]
                    })
