...
function out_application()
{
  ...
  function detail_exces()
  {
    ...
    $critaire11=" AND tracking_data.vitesse>50";
      ...
      $intrant[] = '<a onclick="listing_exces('.$row->VEHICULE_ID.')" href="javascript:;" ><button >
  ...
  function listing_exces($VEHICULE_ID)
  {
    ...
    $var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
    $query_principal='SELECT DISTINCT CODE_COURSE,tracking_data.device_uid,tracking_data.vitesse,tracking_data.date,tracking_data.CODE_COURSE FROM `vehicule` JOIN tracking_data on vehicule.CODE=tracking_data.device_uid  WHERE 1 AND vitesse>50  and vehicule.VEHICULE_ID='.$VEHICULE_ID;
  ...
  function detail_accident()
  {
    ...
    foreach ($fetch_data as $row)
    {
      ...
      $intrant[] ="<strong class='text-dark'/>".$row->desc_chauffeur;
      $intrant[] = '
            <a onclick="listing_acc('.$row->VEHICULE_ID.')" href="javascript:;" ><button  >
  ...
  function listing_acc($VEHICULE_ID)
  {
    ...
    $var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
    $query_principal="SELECT DISTINCT CODE_COURSE,tracking_data.device_uid,tracking_data.vitesse,tracking_data.date,tracking_data.CODE_COURSE FROM `vehicule`  JOIN tracking_data on vehicule.CODE=tracking_data.device_uid  WHERE 1 AND accident=1  and vehicule.VEHICULE_ID='".$VEHICULE_ID."'";
  ...
  $rapp2="<script type=\"text/javascript\">
   ...
   $vehi_code_course=$this->Model->getRequete('SELECT DISTINCT vehicule.VEHICULE_ID,tracking_data.CODE_COURSE FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid WHERE mouvement=0');
    ...
    $vehicule_consomation=$this->Model->getRequete('SELECT DISTINCT vehicule.VEHICULE_ID ,vehicule.PLAQUE,COUNT(mouvement) as  NBR FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid WHERE mouvement=0 AND tracking_data.CODE_COURSE="'.$value['CODE_COURSE'].'" GROUP BY VEHICULE_ID,PLAQUE');
   ...
   $course_voiture=$this->Model->getRequete('SELECT DISTINCT vehicule.CODE,vehicule.PLAQUE,COUNT(DISTINCT `CODE_COURSE`) as  NBR FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid WHERE 1 '.$critere.' GROUP BY CODE,PLAQUE'); 
   ...
   if(!empty($DATE_DEBUT) && !empty($DATE_DAT_FIN))
   {
      ...
      $critere.=' AND users.USER_ID="'.$USER_ID.'"AND date_format(tracking_data.date,"%Y-%m-%d")between "'.$DATE_DEBUT.'" AND "'.$DATE_DAT_FIN.'" ';
    }
     $proce_requete = "CALL `getRequete`(?,?,?,?);";
     $my_selectget_arret_date = $this->getBindParms('id,tracking_data.date', 'tracking_data JOIN vehicule ON vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID', '1 '.$critere.'' , '`id` ASC');
    $my_selectget_arret_date=str_replace('\"', '"', $my_selectget_arret_date);
    $my_selectget_arret_date=str_replace('\n', '', $my_selectget_arret_date);
    $my_selectget_arret_date=str_replace('\"', '', $my_selectget_arret_date);
    $get_arret_date = $this->ModelPs->getRequete($proce_requete, $my_selectget_arret_date);

    $vehicule_exces=$this->Model->getRequeteOne('SELECT COUNT( DISTINCT vehicule.VEHICULE_ID ) as nbr  FROM `tracking_data`  join vehicule on tracking_data.device_uid=vehicule.CODE join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and tracking_data.CODE_COURSE in (SELECT DISTINCT(tracking_data.CODE_COURSE) FROM `tracking_data` JOIN vehicule ON vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and `accident`=1 '.$critere.')') ;

      $vehicule_normal=$this->Model->getRequeteOne('SELECT COUNT( DISTINCT vehicule.VEHICULE_ID ) as nbr  FROM  vehicule  join tracking_data on vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 '.$critere.' and vehicule.VEHICULE_ID not in (SELECT DISTINCT vehicule.VEHICULE_ID   FROM `tracking_data`  join vehicule on tracking_data.device_uid=vehicule.CODE WHERE 1 and tracking_data.CODE_COURSE in (SELECT DISTINCT(tracking_data.CODE_COURSE) FROM `tracking_data` WHERE 1 and `accident`=1))') ;
  ...
  $rapp="<script type=\"text/javascript\">
    ...
    series: [
    {
     ...
     $vehicule_exces=$this->Model->getRequeteOne('SELECT COUNT( DISTINCT vehicule.VEHICULE_ID ) as nbr  FROM `tracking_data`  join vehicule on tracking_data.device_uid=vehicule.CODE join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and tracking_data.CODE_COURSE in (SELECT DISTINCT(tracking_data.CODE_COURSE) FROM `tracking_data` JOIN vehicule ON vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and `vitesse`>50 '.$critere.')') ;

      $vehicule_normal=$this->Model->getRequeteOne('SELECT COUNT( DISTINCT vehicule.VEHICULE_ID ) as nbr  FROM  vehicule  join tracking_data on vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 '.$critere.'  and vehicule.VEHICULE_ID not in (SELECT DISTINCT vehicule.VEHICULE_ID   FROM `tracking_data`  join vehicule on tracking_data.device_uid=vehicule.CODE WHERE 1 and tracking_data.CODE_COURSE in (SELECT DISTINCT(tracking_data.CODE_COURSE) FROM `tracking_data` JOIN vehicule ON vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and `vitesse`>50 '.$critere.'))') ;
  ...
  $rapp2="<script type=\"text/javascript\">
   ...
   // $vehicule_code=$this->Model->getRequete('SELECT DISTINCT vehicule.VEHICULE_ID as ID,vehicule.PLAQUE as NAME,vehicule.KILOMETRAGE as NBR,`latitude`,`longitude`,CODE FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid WHERE 1  GROUP BY ID,NAME,latitude,longitude');
    $vehicule_code=$this->Model->getRequete('SELECT `device_uid` as CODE FROM `tracking_data` JOIN vehicule ON vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and users.USER_ID="'.$USER_ID.'" GROUP BY device_uid');
    $nvldistance=0;
   foreach ($vehicule_code as $key) 
   {
    ...
    $vehicule_consomation=$this->Model->getRequete('SELECT DISTINCT vehicule.VEHICULE_ID ,vehicule.PLAQUE  FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 and users.USER_ID="'.$USER_ID.'" GROUP BY VEHICULE_ID,PLAQUE');
    $donnees10="";
    $donnees101="";
    foreach ($vehicule_consomation as  $value) 
    {
   ...
   $vehi_code_course=$this->Model->getRequete('SELECT DISTINCT vehicule.VEHICULE_ID,tracking_data.CODE_COURSE FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE mouvement=0 and users.USER_ID='.$USER_ID);
    foreach ($vehi_code_course as  $value) 
    {
   ...
   //rapport nbr course par voiture
   $course_voiture=$this->Model->getRequete('SELECT DISTINCT vehicule.CODE,vehicule.PLAQUE,COUNT(DISTINCT `CODE_COURSE`) as  NBR FROM `vehicule`  join tracking_data on vehicule.CODE=tracking_data.device_uid join users ON users.PROPRIETAIRE_ID=vehicule.PROPRIETAIRE_ID WHERE 1 '.$critere.' GROUP BY CODE,PLAQUE'); 
  ...
  function detail_course_voiture()
  {
   ...
   $query_principal='SELECT CODE_COURSE FROM tracking_data WHERE 1';
    ...
    $query_filter=$query_principal.'  '.$critaire.' '.$GROUPby.' '.$search;
