	...
	function tracking_chauffeur_filtres(){
		...
		if(!empty($get_data_arret)){
			...
			foreach ($get_data_arret as $value_get_arret_code) {
				...
				$my_select_date_compare2 = $this->getBindParms('id,tracking_data.date as date_vu,date_format(tracking_data.date,"%H %i") as hour,date_format(tracking_data.date,"%s") as sec,latitude,longitude,date_format(tracking_data.date,"%d %m") as day_month', 'tracking_data', 'CODE_COURSE IS NOT NULL AND CODE_COURSE="'.$value_get_arret_code['CODE_COURSE'].'" ', 'id DESC');
				...
				$my_selectone_element_moins = $this->getBindParms('id', 'tracking_data', ' CODE_COURSE IS NOT NULL AND CODE_COURSE= "'.$value_get_arret_code['CODE_COURSE'].'" AND id > "'.$one_element['id'].'" ' , '`id` ASC');
