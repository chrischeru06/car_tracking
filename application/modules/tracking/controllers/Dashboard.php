...
class Dashboard extends CI_Controller
{
	...
	function tracking_chauffeur_filtres(){
		...
		$get_data_arret = $this->Model->getRequete('SELECT CODE_COURSE FROM tracking_data WHERE md5(device_uid) ="'.$CODE.'" '.$critere.'  GROUP BY CODE_COURSE ORDER BY id ASC');
		...
		if(!empty($get_data_arret)){
			...
			if (!empty($tabl)) 
			{
				foreach ($tabl as $keytabl) 
				{
					...
					// 	</script>';
					// $one='https://api.mapbox.com/geocoding/v5/mapbox.places/'.$lng.','.$lat.'.json?access_token=pk.eyJ1IjoibWFydGlubWJ4IiwiYSI6ImNrMDc0dnBzNzA3c3gzZmx2bnpqb2NwNXgifQ.D6Fm6UO9bWViernvxZFW_A';
					// $voir_endroit='<script>
					// $.get('.$one.',function(data){
					// 	return data;
					// })
					// </script>';

					// print_r($voir_endroit);die();
					if ($keytabl[12]==1) {
