      ...
      <div class="modal-body">
        <form id="traite_dem_form" enctype="multipart/form-data" action="#" method="post">
          <div class="modal-body mb-1">
            <div class="row">
              <input type="hidden" name="ID_RETOUR" id="ID_RETOUR">
          ...
          <div class="modal-footer">
            <input type="button"class="btn btn-outline-primary rounded-pill " type="button" id="btn_add" value="<?=lang('btn_traiter')?>" onclick="save_stat_retour_v();" />
  ...
  <div class="modal" id="Modal_histo_sortie" role="dialog">
    <div class="modal-dialog modal-lg ">
      <div class="modal-content">
        ...
        <h5 class="modal-title">Historique validation retour véhicule </h5>
...
<script>
  ...
  function traiter_demande(ID_RETOUR)
  {
    $('#Modal_traiter').modal('show');

    $('#ID_RETOUR').val(ID_RETOUR);

    $('#errorIS_VALIDATED').html('');
    $.ajax(
    {
      url: "<?= base_url() ?>etat_vehicule/Retour_Vehicule/get_all_statut/",
...
<script>
  ...
  function save_stat_retour_v()
  {
    ...
    if(statut<2)
    {
      ...
      var url="<?= base_url('etat_vehicule/Retour_Vehicule/save_stat_demande')?>";
      $.ajax(
      {
        url: url,
        type: 'POST',
        ...
        success: function(data)
        {
          if(data == 1)
          {
            ...
            {
              window.location.reload('<?=base_url('etat_vehicule/Retour_Vehicule')?>');
            });
          }
          else if(data == 2)
          {
...
<script>
  function get_historique(id)
  {
   ...
   table=$("#table_histo").DataTable({
    ...
    "oreder":[[ 0, 'desc' ]],
    "ajax":{
      url:"<?=base_url()?>etat_vehicule/Retour_Vehicule/getHistorique/"+id,
      type:"POST"
    },
    lengthMenu: [[10,50, 100, row_count], [10,50, 100, "All"]],
    pageLength: 10,
