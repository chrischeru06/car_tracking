            ...
            <div class="col-md-4">
             <table>
              <tr>
                ...
                <td>  
                  <h4 class="text-dark">Mes véhicules</h4>
  ...
  function listing()
  {
    ...
    // var row_count ="1000000";
      ...
      $("#mytable").DataTable({
        ...
        "oreder":[[ 0, 'desc' ]],
        "ajax":{
          url:"<?php echo base_url('etat_vehicule/Vehicule_Affecte_Chauff/listing');?>",
          type:"POST",
...
function save_motif_active() {
  ...
  if (statut==1) {
    ...
    $.ajax(
    {
      url:"<?=base_url()?>etat_vehicule/Vehicule_Affecte_Chauff/active_desactive/"+status,
      type: 'POST',
      dataType:'JSON',
      data: form_data ,
      contentType: false,
      ...
      success: function(data)
      {
       ...
       if(data.status==1)
       {
        ...
        {
         window.location.href='<?=base_url('')?>etat_vehicule/Vehicule_Affecte_Chauff';
       });
      }
    }
  });
...
<script>
  ...
  function assure_controle(VEHICULE_ID_ASSURE_CONTROLE ='',ACTION = '')
  {
    ...
    $.ajax(
    {
      url: "<?= base_url() ?>etat_vehicule/Vehicule_Affecte_Chauff/select_assureur/"+VEHICULE_ID_ASSURE_CONTROLE,
      type: "GET",
      dataType: "JSON",
      success: function(data)
      {
...
<script>
  ...
  if(statut == 1)
  {
    ...
    url = "<?= base_url('etat_vehicule/Vehicule_Affecte_Chauff/save_assure_controle/') ?>";
    $.ajax({
      url: url,
      type: 'POST',
      dataType:'JSON',
...
<script>
 function get_nbr_vehicule()
 {
  ...
  $.ajax({
    url: "<?= base_url() ?>etat_vehicule/Vehicule_Affecte_Chauff/get_nbr_vehicule/" + CHECK_VALIDE,
    type: "POST",
    dataType: "JSON",
    success: function(data) {
