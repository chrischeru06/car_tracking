...
public function get_current_pag($table,$limit, $start,$criteres) {
    $this->db->limit($limit, $start);
    $this->db->join('pub_publication_production','ch_variete.VARIETE_ID=pub_publication_production.VARIETE_ID');
    $this->db->join('ch_filiaire','ch_filiaire.FILIERE_ID=ch_variete.FILIERE_ID');
    $this->db->join('ch_acteur','ch_acteur.ACTEUR_ID=pub_publication_production.ACTEUR_ID');
    $this->db->where($criteres);
    $query = $this->db->get($table);
    $rows = $query->result();
...
public function update_type_acteur($id,$data)
{
...
function calculatePolygonCenter($coordinates) {
    ...
    // Convert average Cartesian coordinates back to spherical coordinates
    $lon = atan2($centroidY, $centroidX);
    $hyp = sqrt($centroidX * $centroidX + $centroidY * $centroidY);
    $lat = atan2($centroidZ, $hyp);
...
//calcul de la superficie du polygone
function calculatePolygonArea($coordinates) {
    ...
    for ($i = 0; $i < $numVertices; $i++) {
        ...
        $totalArea += $radius * $radius * $c;
    }

    return abs($totalArea);
}

//Verifie si un point est in or out polygon
